...
def handle_standard_lookup(key, value, target_env, db_name, g_env_map, lookup_table, entity_name=None):
    """
    STANDARD FALLBACK ENGINE:
    Evaluates tier preferences flatly across any key present in your mapping configuration.
    Includes case-insensitive fallback resolution for cross-environment string mismatches.
    """
    ...
    if key in g_env_map and isinstance(g_env_map[key], dict):
        ...
        # TIER 1: Highest Preference -> [db][entity][value]
        if entity_name:
            entity_lower = entity_name.lower()
            entity_bracket_exact = f"[{db_name}][{entity_name}][{val_str}]"
            entity_bracket_lower = f"[{db_lower}][{entity_lower}][{val_lower}]"
            
            if entity_bracket_exact in category_map and target_env in category_map[entity_bracket_exact]:
                return category_map[entity_bracket_key][target_env]
                
            # Case-insensitive check for Tier 1
            for k, v in category_map.items():
                if k.lower() == entity_bracket_lower and target_env in v:
                    return v[target_env]
                    
        # TIER 2: Secondary Preference -> [db][value]
        db_bracket_exact = f"[{db_name}][{val_str}]"
        db_bracket_lower = f"[{db_lower}][{val_lower}]"
        
        if db_bracket_exact in category_map and target_env in category_map[db_bracket_exact]:
            return category_map[db_bracket_exact][target_env]
            
        # Case-insensitive check for Tier 2
        for k, v in category_map.items():
            if k.lower() == db_bracket_lower and target_env in v:
                return v[target_env]
                
        # TIER 3: Lower Preference / Shared Fallback -> Direct category lookup
        if val_str in category_map and target_env in category_map[val_str]:
            return category_map[val_str][target_env]
            
        # Case-insensitive check for Tier 3
        for k, v in category_map.items():
            if k.lower() == val_lower and target_env in v:
                return v[target_env]

    # TIER 4: Global Matrix Fallback Lookups (Case-Insensitive fallback matching)
    if val_str in lookup_table and target_env in lookup_table[val_str]:
        return lookup_table[val_str][target_env]
    ...
    if val_lower.endswith('_nt1'):
        cleaned_val = val_str[:-4]
        cleaned_lower = val_lower[:-4]
        if cleaned_val in lookup_table and target_env in lookup_table[cleaned_val]:
            return lookup_table[cleaned_val][target_env]
        for k, v in lookup_table.items():
            if k.lower() == cleaned_lower and target_env in v:
                return v[target_env]
...
def update_entities_only(entities_node, target_env, db_name, g_env_map, lookup_table):
    """
    SEPARATE FUNCTION: Processes and updates values inside the entities block level flatly.
    Includes inner key traversing to handle dynamic array structure variations smoothly.
    """
    if isinstance(entities_node, list):
            ...
            if isinstance(item, dict):
                ...
                for key, value in item.items():
                    if key == "source_entity":
                        continue
                    if isinstance(value, str):
                        item[key] = handle_standard_lookup(
                            key, value, target_env, db_name, g_env_map, lookup_table, entity_name=entity_name
                        )
                    elif isinstance(value, (dict, list)):
                        # Cleanly handle inner structure modifications if fields are nested within an entity array
                        process_nested_entity_elements(value, target_env, db_name, g_env_map, lookup_table, entity_name)
        ...
        for key, value in entities_node.items():
            if key == "source_entity":
                continue
            if isinstance(value, str):
                entities_node[key] = handle_standard_lookup(
                    key, value, target_env, db_name, g_env_map, lookup_table, entity_name=entity_name
                )


def process_nested_entity_elements(node, target_env, db_name, g_env_map, lookup_table, entity_name):
    """
    Helper function to safely evaluate and map nested dictionary/list values within an individual entity object.
    """
    if isinstance(node, dict):
        for k, v in node.items():
            if isinstance(v, str):
                node[k] = handle_standard_lookup(k, v, target_env, db_name, g_env_map, lookup_table, entity_name)
            elif isinstance(v, (dict, list)):
                process_nested_entity_elements(v, target_env, db_name, g_env_map, lookup_table, entity_name)
    elif isinstance(node, list):
        for element in node:
            if isinstance(element, dict):
                process_nested_entity_elements(element, target_env, db_name, g_env_map, lookup_table, entity_name)
