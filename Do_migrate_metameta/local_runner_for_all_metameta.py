...
# -----------------------------------------------------------------
def handle_destination_database(val, src_metameta, g_env_map, env_name_clean, context=None):
...
def handle_destination_warehouse(val, src_metameta, g_env_map, env_name_clean, context=None):
...
def handle_source_database(val, src_metameta, g_env_map, env_name_clean, context=None):
    if src_metameta.get('source_type', '').lower() == "snowflake":
        if val.lower() in g_env_map.get("Database", {}):
            return g_env_map["Database"][val.lower()][env_name_clean].upper()
    else:
...
def handle_source_warehouse(val, src_metameta, g_env_map, env_name_clean, context=None):
    if src_metameta.get('source_type', '').lower() == "snowflake":
        if val.lower() in g_env_map.get("Warehouse", {}):
            return g_env_map["Warehouse"][val.lower()][env_name_clean].upper()
    return val

def handle_pattern_fields(val, src_metameta, g_env_map, env_name_clean, db_name_lower, context=None):
...
# -----------------------------------------------------------------
def replace_recursively(node, src_metameta, g_env_map, env_name_clean, db_name_lower, current_key=None, parent_dict=None):
    conditional_rules = {
        'destination_database': lambda v, ctx: handle_destination_database(v, src_metameta, g_env_map, env_name_clean, ctx),
        'destination_warehouse': lambda v, ctx: handle_destination_warehouse(v, src_metameta, g_env_map, env_name_clean, ctx),
        'source_database': lambda v, ctx: handle_source_database(v, src_metameta, g_env_map, env_name_clean, ctx),
        'source_warehouse': lambda v, ctx: handle_source_warehouse(v, src_metameta, g_env_map, env_name_clean, ctx),
        'ftp_host_name': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx),
        'default_ftp_host_name': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx),
        'to_uat': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx),
        'default_ftp_root_folder': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx),
        'root_folder': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx),
        'workspace_id': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx),
        'resource_id': lambda v, ctx: handle_pattern_fields(v, src_metameta, g_env_map, env_name_clean, db_name_lower, ctx)
    }

    if isinstance(node, dict):
        return {k: replace_recursively(v, src_metameta, g_env_map, env_name_clean, db_name_lower, current_key=k, parent_dict=node) for k, v in node.items()}
    elif isinstance(node, list):
        return [replace_recursively(item, src_metameta, g_env_map, env_name_clean, db_name_lower, current_key=current_key, parent_dict=parent_dict) for item in node]
...
# -----------------------------------------------------------------
# LOCAL TESTING SUITE ENTRANCE
# -----------------------------------------------------------------
if __name__ == "__main__":
    print(">>> Beginning local emulation framework run...")
    
    os.makedirs(os.path.join(SOURCE_ROOT, "my_database"), exist_ok=True)
    os.makedirs(os.path.join(TARGET_ROOT, "my_database"), exist_ok=True)
    ...
    metameta_file = os.path.join(SOURCE_ROOT, "my_database", "my_database_metameta.json")
    
    if not os.path.exists(global_map_file) or not os.path.exists(metameta_file):
        print("\n[!] Setup Needed: Before running, please drop your JSON contents into the following paths:")
        print(f" -> Global Map Location: {global_map_file}")
        print(f" -> Metameta File Location: {metameta_file}\n")
    else:
        db_name_target = local_execution_parameters["db_names"]
        ...
        print(f">>> Execution completed. Check '{TARGET_ROOT}/{db_name_target}' folder for outputs.")import os
import json
import logging
from datetime import datetime
...
if __name__ == "__main__":
    ...
    # 1. Critical Safeguard: Verify the Master Global Map exists before scanning folders
    if not os.path.exists(global_map_file):
        ...
        # 2. Automatically scan the SOURCE_ROOT for all subdirectories (Databases)
        if os.path.exists(SOURCE_ROOT):
            source_items = os.listdir(SOURCE_ROOT)
            # Filter out files (like global_environment_map.json) to keep ONLY folders
        ...
        if not db_folders:
            ...
            print(" -> Create a folder (e.g., 'my_database') and place its metameta.json inside it.\n")
            ...
            for db_name_target in db_folders:
                print(f"\n[Engine] Starting migration compilation loop for database: '{db_name_target}'")
                ...
                # 4. Trigger Migration Strategy Pipeline for this specific database folder
                run_migrate_metameta(
                    db_name_target,
                    "local_src", "local_tgt",
                    local_execution_parameters["src_container"],
                ...
                print(f" -> [Success] Output generated at: {target_db_dir}/{db_name_target}_metameta.json")
                
            print("\n>>> All discovered source folder loops successfully executed.")
